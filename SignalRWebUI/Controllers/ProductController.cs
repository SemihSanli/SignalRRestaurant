...
namespace SignalRWebUI.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7205/api/Product/ProductListWithCategory");
        ...
        public async Task<IActionResult> CreateProduct(CreateProductDTOs createProductDTOs)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7205/api/Product", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7205/api/Product/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> UpdateProduct(int id)
        {
            ...
            var responseMessage1 = await client1.GetAsync("https://localhost:7205/api/Category");
            ...
            ViewBag.v = values2;
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:7205/api/Product/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateProduct(UpdateProductDTOs updateProductDTOs)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7205/api/Product/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
